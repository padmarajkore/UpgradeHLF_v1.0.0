...
# stopping the 'peer0.org2.example.com' container
...
cp -r organizations/peerOrganizations/org2.example.com backup/organizations/peerOrganizations

# fetching the REPOSITORY (image_name) for the peer0.org2.example.com chaincode image
CC_IMAGE=$(docker images | grep dev-peer0.org2.example.com | awk '{print $1}')

# removing the chaincode image
docker rmi -f $CC_IMAGE

# removing the 'peer0.org2.example.com' container
docker rm -f peer0.org2.example.com

# inside 'test-network/compose/compose-test-net.yaml' file
# modify 'image' field of 'peer0.org2.example.com' service
...
# inside 'test-network/compose/compose-test-net.yaml' file
# modify 'volumes' section of 'peer0.org2.example.com' service
# Define the new volumes section
new_volumes="      - ../backup/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com:/etc/hyperledger/fabric
...
awk -v new_volumes="$new_volumes" '
  /^services:/ { services=1 }
  services && /^  peer0.org2.example.com:/ { peer0_org2=1 }
  peer0_org2 && /^    volumes:/ {
    print
    while (getline) {
      if (/^[[:space:]]*-/) {
...
# add these in the 'environment' section of 'peer0.org2.example.com' service
...
# launching the 'peer0.org2.example.com' service using the 'compose-test-net.yaml' file
# # docker-compose -f configuration_file_path up -d service_name
docker-compose -f compose/compose-test-net.yaml up -d peer0.org2.example.com

# inspecting containers after upgrading the 'peer0.org2.example.com' service 
docker ps -a